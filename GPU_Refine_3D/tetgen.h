...
public:
	...
	// Timers for Delaunay refinements
	double refine_times[3];

	// Internal neighbor lists
	//  Output only
	int interneigh;
	REAL* interpointradius;
	...
	void initialize()
	{
		...
		refine_times[0] = refine_times[1] = refine_times[2] = 0;

		interneigh = 0;
		interpointradius = (REAL *)NULL;
		intertri2tetlist = (int *)NULL;
		intertri2trilist = (int *)NULL;
		intertet2tetlist = (int *)NULL;
...
#define fastlookup(pool, index) \
  (void *) ((pool)->toparray[(index) >> (pool)->log2objectsperblock] + \
	...
	void outinterpointradius(tetgenio*);
	void outintertri2tetlist(tetgenio*);
	void outintertri2trilist(tetgenio*);
	void outintertet2tetlist(tetgenio*);
